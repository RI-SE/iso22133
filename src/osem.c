...
static inline int64_t le48toh_signed(const void *p)
{
	const uint8_t *b = (const uint8_t *)p;

	// Build 48-bit little-endian value into the low bits of a 64-bit register
	uint64_t v = ((uint64_t)b[0]) | ((uint64_t)b[1] << 8) | ((uint64_t)b[2] << 16) | ((uint64_t)b[3] << 24) | ((uint64_t)b[4] << 32) | ((uint64_t)b[5] << 40);

	// Sign-extend from bit 47: move sign to bit 63, then arithmetic right shift
	return (int64_t)((int64_t)(v << 16) >> 16);
}
	...
	const char debug)
{
	...
	uint32_t msgLen = sizeof(HeaderType) + sizeof(OSEMIDType) + sizeof(OSEMOriginType) + sizeof(OSEMDateTimeType) + sizeof(OSEMAccuracyRequirementsType) + 4 * 2 * sizeof(uint16_t) + sizeof(FooterType);
	...
	OSEMData.origin.latitude = objectSettings->coordinateSystemOrigin.isLatitudeValid ? (int64_t)(objectSettings->coordinateSystemOrigin.latitude_deg * LATITUDE_ONE_DEGREE_VALUE)
																					  : LATITUDE_UNAVAILABLE_VALUE;
	OSEMData.origin.longitude = objectSettings->coordinateSystemOrigin.isLongitudeValid ? (int64_t)(objectSettings->coordinateSystemOrigin.longitude_deg * LONGITUDE_ONE_DEGREE_VALUE)
																						: LONGITUDE_UNAVAILABLE_VALUE;
	OSEMData.origin.altitude = objectSettings->coordinateSystemOrigin.isAltitudeValid ? (int32_t)(objectSettings->coordinateSystemOrigin.altitude_m * ALTITUDE_ONE_METER_VALUE)
	...
	OSEMData.requirements.maxWayDeviation = (uint16_t)(objectSettings->maxDeviation.position_m * MAX_WAY_DEVIATION_ONE_METER_VALUE);
	OSEMData.requirements.maxLateralDeviation = (uint16_t)(objectSettings->maxDeviation.lateral_m * MAX_LATERAL_DEVIATION_ONE_METER_VALUE);
	OSEMData.requirements.maxYawDeviation = (uint16_t)(objectSettings->maxDeviation.yaw_rad * 180.0 / M_PI * MAX_YAW_DEVIATION_ONE_DEGREE_VALUE);
	OSEMData.requirements.maxPositionError = (uint16_t)(objectSettings->minRequiredPositioningAccuracy_m * MIN_POSITIONING_ACCURACY_ONE_METER_VALUE);
	OSEMData.requirements.heabTimeout = (uint16_t)((objectSettings->heabTimeout.tv_sec + objectSettings->heabTimeout.tv_usec / 1000000.0) * COMMUNICATION_TIMEOUT_ONE_SECOND_VALUE);
	...
	memcpy(p, &OSEMData.idStructValueID, sizeof(OSEMData.idStructValueID) + sizeof(OSEMData.idStructContentLength) + sizeof(OSEMData.ids));
	p += sizeof(OSEMData.idStructValueID) + sizeof(OSEMData.idStructContentLength) + sizeof(OSEMData.ids);

	// Special handling of 48 bit values
	memcpy(p, &OSEMData.originStructValueID, sizeof(OSEMData.originStructValueID) + sizeof(OSEMData.originStructContentLength));
	...
	memcpy(p, &OSEMData.origin.altitude, sizeof(OSEMData.origin.altitude) + sizeof(OSEMData.origin.rotation) + sizeof(OSEMData.origin.coordinateSystem));
	p += sizeof(OSEMData.origin.altitude) + sizeof(OSEMData.origin.rotation) + sizeof(OSEMData.origin.coordinateSystem);

	// Copy rest of struct (excluding footer) directly into buffer since no more byte anomalies remain
	memcpy(p, &OSEMData.dateTimeStructValueID, sizeof(OSEMData.dateTimeStructValueID) + sizeof(OSEMData.dateTimeStructContentLength) + sizeof(OSEMData.timestamp) + sizeof(OSEMData.accReqStructValueID) + sizeof(OSEMData.accReqStructContentLength) + sizeof(OSEMData.requirements));
	p += sizeof(OSEMData.dateTimeStructValueID) + sizeof(OSEMData.dateTimeStructContentLength) + sizeof(OSEMData.timestamp) + sizeof(OSEMData.accReqStructValueID) + sizeof(OSEMData.accReqStructContentLength) + sizeof(OSEMData.requirements);

	if (timeServerUsed)
	{
		memcpy(p, &OSEMData.timeServerStructValueID, sizeof(OSEMData.timeServerStructValueID) + sizeof(OSEMData.timeServerStructContentLength) + sizeof(OSEMData.timeserver));
		p += sizeof(OSEMData.timeServerStructValueID) + sizeof(OSEMData.timeServerStructContentLength) + sizeof(OSEMData.timeserver);
	}
	...
	const char debug)
{
	...
	while ((size_t)(p - osemDataBuffer) < OSEMData.header.messageLength + sizeof(OSEMData.header))
	{
		...
		case VALUE_ID_OSEM_ORIGIN_STRUCT:
			...
			memcpy(&OSEMData.origin.altitude, p + sizeof(OSEMData.origin.latitude) + sizeof(OSEMData.origin.longitude) - 2 * SizeDifference64bitTo48bit, sizeof(OSEMData.origin.altitude));
			memcpy(&OSEMData.origin.rotation, p + sizeof(OSEMData.origin.latitude) + sizeof(OSEMData.origin.longitude) + sizeof(OSEMData.origin.altitude) - 2 * SizeDifference64bitTo48bit, sizeof(OSEMData.origin.rotation));
			memcpy(&OSEMData.origin.coordinateSystem, p + sizeof(OSEMData.origin.latitude) + sizeof(OSEMData.origin.longitude) + sizeof(OSEMData.origin.altitude) + sizeof(OSEMData.origin.rotation) - 2 * SizeDifference64bitTo48bit,
				   sizeof(OSEMData.origin.coordinateSystem));

			OSEMData.origin.latitude = le48toh_signed(p);
			OSEMData.origin.longitude = le48toh_signed(p + 6);
	...
	ObjectSettingsType *ObjectSettingsData)
{
	...
	ObjectSettingsData->coordinateSystemOrigin.isLatitudeValid =
		OSEMData->originStructValueID && OSEMData->origin.latitude != LATITUDE_UNAVAILABLE_VALUE;
	ObjectSettingsData->coordinateSystemOrigin.isLongitudeValid =
		OSEMData->originStructValueID && OSEMData->origin.longitude != LONGITUDE_UNAVAILABLE_VALUE;
	ObjectSettingsData->coordinateSystemOrigin.isAltitudeValid =
		OSEMData->originStructValueID && OSEMData->origin.altitude != ALTITUDE_UNAVAILABLE_VALUE;
	ObjectSettingsData->coordinateSystemRotation_rad =
		OSEMData->originStructValueID ? OSEMData->origin.rotation / ROTATION_ONE_DEGREE_VALUE * M_PI / 180.0 : 0.0;
	ObjectSettingsData->coordinateSystemType = OSEMData->originStructValueID
												   ? OSEMData->origin.coordinateSystem
	...
	ObjectSettingsData->maxDeviation.position_m = OSEMData->requirements.maxWayDeviation != MAX_WAY_DEVIATION_UNAVAILABLE_VALUE ? OSEMData->requirements.maxWayDeviation / MAX_WAY_DEVIATION_ONE_METER_VALUE : 1000000.0;
	ObjectSettingsData->maxDeviation.lateral_m = OSEMData->requirements.maxLateralDeviation != MAX_LATERAL_DEVIATION_UNAVAILABLE_VALUE ? OSEMData->requirements.maxLateralDeviation / MAX_LATERAL_DEVIATION_ONE_METER_VALUE : 1000000.0;
	ObjectSettingsData->maxDeviation.yaw_rad = OSEMData->requirements.maxYawDeviation != MAX_YAW_DEVIATION_UNAVAILABLE_VALUE ? OSEMData->requirements.maxYawDeviation / MAX_YAW_DEVIATION_ONE_DEGREE_VALUE * M_PI / 180.0 : 2 * M_PI;
	ObjectSettingsData->minRequiredPositioningAccuracy_m = OSEMData->requirements.maxPositionError != MIN_POSITIONING_ACCURACY_NOT_REQUIRED_VALUE ? OSEMData->requirements.maxPositionError / MIN_POSITIONING_ACCURACY_ONE_METER_VALUE : 1000000.0;
	...
	ObjectSettingsData->heabTimeout.tv_usec = (OSEMData->requirements.heabTimeout - ObjectSettingsData->heabTimeout.tv_sec * COMMUNICATION_TIMEOUT_ONE_SECOND_VALUE) / COMMUNICATION_TIMEOUT_ONE_SECOND_VALUE * 1000000;
